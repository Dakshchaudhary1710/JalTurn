...
const { waterGroups, farmers, plots, waterTurns, disputes, auditLogs, notifications } = require('../data/seedData');
...
const TieBreakLog = require('../models/TieBreakLog');

const seedDatabase = async () => {
  try {
    const groupCount = await WaterGroup.countDocuments();
    if (groupCount === 0) {
      const clean = (arr) => arr ? arr.map(({ _id, ...rest }) => rest) : [];
      ...
      if (notifications && notifications.length > 0) {
        await Notification.insertMany(clean(notifications));
      }
